...
public class CutAndFill : GH_Component {
  ...
  public CutAndFill()
      : base("CutFill",
             "batCut",
             "Compute the 'Cut and Fill' difference between two given terrains.",
             "BAT",
             "Analysis") {}
  ...
  protected override void RegisterInputParams(GH_InputParamManager pManager) {
    ...
    pManager.AddMeshParameter(
        "Existing", "E", "Existing terrain mesh covering the analyzed area.", GH_ParamAccess.item);
    pManager.AddMeshParameter(
        "Proposed", "P", "Proposed terrain mesh covering the analyzed area.", GH_ParamAccess.item);
    pManager.AddIntegerParameter("GridSize",
                                 "G",
  ...
  protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
    pManager.AddMeshParameter("AnalysisMesh", "M", "Mesh showing the cut and fill analysis.", GH_ParamAccess.item);
    pManager.AddMeshParameter(
        "CutMeshes", "CM", "Mesh faces in cut areas (where material is removed).", GH_ParamAccess.list);
    pManager.AddMeshParameter(
        "FillMeshes", "FM", "Mesh faces in fill areas (where material is added).", GH_ParamAccess.list);
    pManager.AddNumberParameter(
        "CutVolume", "C", "Volume of material that needs to be excavated.", GH_ParamAccess.list);
    pManager.AddNumberParameter("FillVolume", "F", "Volume of material that needs to be added.", GH_ParamAccess.list);
    pManager.AddNumberParameter("NetVolume",
                                "N",
...
#region data input& validation
    ...
    Mesh analysisMesh = CreateAnalysisMesh(
        gridCenters, existingHeights, heightDifferences, boundaries, gridSize, combinedBbox, selectedColorMap);
...
#region data output
  ...
  private Mesh CreateAnalysisMesh(List<Point3d> gridCenters,
                                  List<double> existingHeights,
    ...
    for (int i = 0; i < xCount - 1; i++) {
      for (int j = 0; j < yCount - 1; j++) {
        ...
        if (v0 < analysisMesh.Vertices.Count && v1 < analysisMesh.Vertices.Count && v2 < analysisMesh.Vertices.Count &&
            v3 < analysisMesh.Vertices.Count) {
  ...
  /// </summary>
  private (List<Mesh> cutMeshes, List<Mesh> fillMeshes) ExtractCutFillMeshes(
      Mesh analysisMesh, List<double> heightDifferences, List<Curve> boundaries, double gridSize, BoundingBox bbox) {
    ...
    for (int faceIdx = 0; faceIdx < analysisMesh.Faces.Count; faceIdx++) {
      ...
      if (v0 >= heightDifferences.Count || v1 >= heightDifferences.Count || v2 >= heightDifferences.Count ||
          (isQuad && v3 >= heightDifferences.Count)) {
        continue;
      }
          ...
          isQuad ? new Point3d((analysisMesh.Vertices[v0].X + analysisMesh.Vertices[v1].X +
                               ...
                               (analysisMesh.Vertices[v0].Z + analysisMesh.Vertices[v1].Z +
                                analysisMesh.Vertices[v2].Z + analysisMesh.Vertices[v3].Z) /
                                   4.0)
                 : new Point3d(
                       (analysisMesh.Vertices[v0].X + analysisMesh.Vertices[v1].X + analysisMesh.Vertices[v2].X) / 3.0,
                       (analysisMesh.Vertices[v0].Y + analysisMesh.Vertices[v1].Y + analysisMesh.Vertices[v2].Y) / 3.0,
                       (analysisMesh.Vertices[v0].Z + analysisMesh.Vertices[v1].Z + analysisMesh.Vertices[v2].Z) / 3.0);

      bool insideBoundary = false;
      foreach (Curve boundary in boundaries) {
        if (boundary != null && boundary.IsPlanar()) {
          if (boundary.Contains(new Point3d(faceCenter.X, faceCenter.Y, 0), Plane.WorldXY, 0.01) !=
