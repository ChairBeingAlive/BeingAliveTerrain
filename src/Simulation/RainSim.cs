...
public class RainSim : GH_Component {
  public RainSim()
      : base("RainSimulation", "batRainSim", "Waterflow simulation on the target terrain.", "BAT", "Simulation") {}
  ...
  protected override void RegisterInputParams(GH_InputParamManager pManager) {
    pManager.AddMeshParameter("Mesh", "M", "Terrain to run the rain simulation.", GH_ParamAccess.item);
    pManager.AddPointParameter(
        "Start", "P", "Start points for the rain simulation on or above the target terrain.", GH_ParamAccess.list);
    pManager.AddIntegerParameter(
        "Steps", "S", "Maximum number of steps to run for the simulation.", GH_ParamAccess.item, 100);
    pManager.AddNumberParameter("StepSize", "D", "Distance to move in each simulation step.", GH_ParamAccess.item, 1.0);
  ...
  protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
    pManager.AddCurveParameter("Flowline",
                               "C",
  ...
  /// </summary>
  private List<Point3d> SimulateWaterFlowGradient(Mesh terrain, Point3d startPoint, int maxSteps, double stepSize) {
